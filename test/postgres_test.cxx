...
class TestDatabase : public Postgres {
    ...
    private:
        void DoPostConnect() noexcept override {
            // Create tables
            SilentQuery("CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, name TEXT NOT NULL, email TEXT NOT NULL UNIQUE);");
            SilentQuery("CREATE TABLE IF NOT EXISTS products (id SERIAL PRIMARY KEY, name TEXT NOT NULL, price DOUBLE PRECISION NOT NULL);");
            SilentQuery("CREATE TABLE IF NOT EXISTS orders (id SERIAL PRIMARY KEY, user_id INTEGER, product_id INTEGER, quantity INTEGER NOT NULL, FOREIGN KEY (user_id) REFERENCES users(id), FOREIGN KEY (product_id) REFERENCES products(id));");
            SilentQuery("CREATE TABLE IF NOT EXISTS blobs (id SERIAL PRIMARY KEY, data BYTEA);");

            // Insert data
            // Ensure a clean slate for tests
            SilentQuery("TRUNCATE TABLE users, products, orders, blobs RESTART IDENTITY;");
            ...
            PrepareSTMT("insert_blob", "INSERT INTO blobs (data) VALUES ($1);");
